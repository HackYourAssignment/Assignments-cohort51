...
export function tellFortune(x, y, s, z) {
  const numKids = selectRandomly(x);
...
function main() {
  const numKids = [0, 1, 3, 7, 4];

  const partnerNames = ['karen', 'sarah', 'Judy', 'Hied', 'Ala'];

  const locations = ['Amsterdam', 'Utrecht', 'Rotterdam', 'paris', 'rome'];

  const jobTitles = ['salesman', 'postman', 'carpenter', 'CEO', 'unemployed'];
